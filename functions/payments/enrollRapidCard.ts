...
async function login(): Promise<string> {
  ...
  console.log('Rapid pre-login status:', preRes.status);
  if (!preRes.ok) {
  ...
  console.log('Rapid login status:', res.status);
  if (!res.ok) {
...
async function registerCard(bearer: string, card: Record<string, string>) {
  ...
  const text = await res.text();
  console.log('Rapid register response:', res.status);
  ...
  if (data.responseCode !== 0) {
    throw new Error(
      `Register error (responseCode ${data.responseCode}): ${data.responseDescription || data.exceptionMessage || 'unknown'}`
    );
  }
  return data.data || {};
}
