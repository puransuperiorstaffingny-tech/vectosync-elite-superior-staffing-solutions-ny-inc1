  ...
  try {
    ...
    if (syncType === 'all' || syncType === 'expenses') {
      ...
      for (const exp of expenseReports) {
        ...
        if (emp && emp.location) {
          await base44.asServiceRole.entities.ExpenseReport.update(exp.id, { employee_email: emp.email });
          synced++;
        }
      }
    ...
    if (syncType === 'all' || syncType === 'requests') {
      synced = 0;
      for (const empReq of employeeRequests) {
        const emp = employees.find(e => e.id === empReq.employee_id);
        if (emp && emp.email !== empReq.employee_email) {
          await base44.asServiceRole.entities.EmployeeRequest.update(empReq.id, { employee_email: emp.email });
          synced++;
        }
      }
    ...
    return Response.json({ 
      success: true, 
      message: `Data sync completed for ${syncType}`,
