...
Deno.serve(async (req) => {
  try {
    ...
    const employees = await base44.asServiceRole.entities.Employee.filter({ email: employee_email });
    ...
    const depts = await base44.asServiceRole.entities.Department.filter({ name: employeeDept });
    ...
    if (!managerEmail) {
      ...
      return Response.json({ success: true, message: 'Expense logged but no manager notification sent' });
    }
...
<body style="font-family: Arial, sans-serif; color: #333;">
  ...
  <p><strong>Category:</strong> ${exp.category || 'N/A'}</p>
  <p><strong>Date:</strong> ${exp.date || new Date().toISOString().slice(0, 10)}</p>
  <hr />
  <p>Please review this expense report in the system.</p>
</body>
</html>
    `;
