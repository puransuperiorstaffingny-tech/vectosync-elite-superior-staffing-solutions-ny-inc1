...
Deno.serve(async (req) => {
  try {
    ...
    const employee = await base44.entities.Employee.filter({ employee_id_number: data.employee_id });
    if (employee.length === 0) return Response.json({ success: true });
    ...
    const emailPromises = adminUsers.map(admin =>
      base44.integrations.Core.SendEmail({
        to: admin.email,
        subject: `Time Entry Submitted by ${data.employee_name}`,
        body: `
